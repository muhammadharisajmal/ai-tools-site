...
// Synchronized with Homepage & Signup: Premium Dark Navy background with radial glows & grid overlay
...
function SignInFormContent() {
  ...
  // Handle URL callback errors if redirected back by Auth.js
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await signIn("credentials", {
        ...
        password,
        redirect: false,
        callbackUrl: callbackUrlParam || (isAdminAccess ? "/admin" : "/dashboard"),
      });
      ...
      if (res?.error) {
        ...
        router.push(res.url || (isAdminAccess ? "/admin" : "/dashboard"));
        router.refresh();
      }
    } catch (err) {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center font-sans bg-slate-950 text-slate-100 selection:bg-purple-500 selection:text-white px-4 py-12 antialiased">
      ...
      <main className="w-full flex items-center justify-center relative z-10">
        {/* Glassmorphism login card with subtle glow framing */}
        <div className="relative w-full max-w-md group">
          ...
          <div className="relative w-full bg-slate-900/80 backdrop-blur-2xl border border-slate-800 rounded-3xl shadow-2xl flex flex-col px-7 py-9 sm:px-10 sm:py-11 transition-all duration-300">
            ...
            <div className="text-center mb-7">
              ...
              {/* STEP 3: Dynamic Admin Portal Badge & Titles */}
              {isAdminAccess && (
            ...
            {(errorMessage || callbackError) && (
              ...
              >
                ...
                <span>
                  {errorMessage ||
                    (callbackError === "OAuthSignin" || callbackError === "OAuthCallback"
            ...
            <form className="w-full flex flex-col gap-4.5" onSubmit={handleSubmit} noValidate>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <Link
                    ...
                    className="text-xs font-medium text-purple-400 hover:text-purple-300 transition-colors focus:outline-none focus:underline"
                  >
                    Forgot password?
                  </Link>
                </div>
              ...
              {/* Dynamic Primary Submit Button Label */}
              <button
                type="submit"
                disabled={isSubmitting || isGoogleLoading}
              ...
              >
                {isSubmitting ? (
                  ...
                  <span>{isAdminAccess ? "Sign In to Admin Portal" : "Sign In"}</span>
                )}
              </button>
            </form>
